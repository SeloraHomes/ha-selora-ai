...
def test_entity_id_local_part_beats_domain_only() -> None:
    """Keyword `light` should NOT match every `light.*` entity via its domain prefix —
    the user almost never means 'any light', they mean a specific one."""
    plain_light = _entity("light.x", "Lamp")  # only the domain matches "light"
    ...
    # plain_light has only domain prefix → eid_local "x" → 0 score; should NOT be kept
    assert plain_light not in kept
    assert named_match in kept
...
def test_low_context_keywords_drops_stopwords() -> None:
    ...
    assert _low_context_keywords("turn on the porch light please") == {"turn", "porch", "light"}
