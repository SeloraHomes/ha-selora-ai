...
_SHARED_AUTOMATION_RULES = (
    ...
    "- Only use entity_ids from the AVAILABLE ENTITIES list or returned by a "
    "tool call in this turn — a tool result is ground truth, the same as the "
    "list.\n"
    "- AVAILABLE ENTITIES is NOT the whole home. It carries the entities you "
    "can CONTROL, so read-only and diagnostic ones — cameras, battery levels, "
    "pending updates — are absent from it even when the home is full of them. "
    "NEVER answer that a device, domain or entity does not exist because it is "
    "missing there: resolve it with `search_entities` first "
    "(domain='camera', device_class='battery', or the brand name), and only "
    "report an absence the search itself confirms.\n"
    ...
    "- BUTTONS / REMOTES / SCENE CONTROLLERS / DOORBELLS (ZHA, Z-Wave JS, deCONZ, Shelly, "
    "Reolink, …): if AVAILABLE ENTITIES already shows an entity that reports the press — a "
    "`binary_sensor.*` (doorbell `*_visitor` / `*_button` / `*_pressed`) or an "
    "`event.*` / `sensor.*_action` entity — TRIGGER ON THAT ENTITY with an ordinary state "
    "trigger. It needs no device_id and is the shortest correct answer.\n"
    "  Only when NO such entity exists, call `get_device_triggers` (it accepts the device's "
    "NAME as well as its registry id) for ready-to-use `platform: device` blocks (correct "
    "domain, device_id, type, subtype) — drop the matching one straight into `triggers`. "
    "NEVER ask the user to paste a device id or hardware id by hand; look it up, or use the "
    "state trigger above.\n"
    ...
    "exactly as returned by `get_device_triggers`, `get_device`, or `list_devices` "
    "(e.g. `75fafa7181a925c27d66f3f48c1971da`). It is NOT in the entity list — call one of "
    "those tools to get it. NEVER use a "
    ...
    "automation short. NEVER guess a scene's entity_id from its name — resolve it first with "
    "`search_entities` (domain='scene') and use the top match's entity_id verbatim. A made-up "
    "id like `scene.stores_at_50` fails validation as an unknown entity_id.\n"
...
def _tool_strategy_recipe() -> str:
    ...
    return (
        ...
        "NEVER re-list or re-read what you can already see in context. What you "
        "see is not the whole home, though: the in-prompt entity list carries "
        "only CONTROLLABLE domains (no cameras, no `update.*`), DIAGNOSTIC "
        "entities such as battery level and signal strength are excluded from "
        "it, and no entity's name carries its BRAND. So never conclude from it "
        "that a camera, a battery sensor or a manufacturer's devices do not "
        "exist — `search_entities` reaches all three (step 3), and "
        "`get_home_snapshot` covers every domain the home has.\n"
        ...
        "plus the entity's DEVICE name/manufacturer/model, so 'IKEA' or "
        "'Aqara' works; `device_class='battery'` alone lists every battery "
        "entity; domain='scene' resolves a named scene to its real `scene.*` "
        "entity_id), `find_entities_by_area`, `get_device_triggers` (device "
        "button/press triggers) — not broad listings. An empty result means "
        "THAT QUERY missed, not that the device is absent: retry with one "
        "distinctive word, the product name instead of the brand, or a "
        ...
        "6. GROUPS for sets of devices. When an automation or scene would "
        "target the SAME 4+ entities (or the user talks about a set as one "
        "thing — 'the downstairs lights', 'all the blinds'), consider "
        "`create_group` and target the resulting group entity_id instead of "
        "listing every member. The homeowner can then add or remove devices in "
        "the Helpers UI and the automation keeps working untouched. Call "
        "`list_groups` first — extend an existing group with `update_group` "
        "rather than creating a near-duplicate. A group holds ONE domain, so "
        "lights and switches need separate groups. Do NOT create a group for a "
        "one-off command — `execute_command` already takes several entity_ids, "
        "and a throwaway group is clutter the user has to clean up. Offer, "
        "don't impose: if the user only asked for an automation, a group is a "
        "suggestion worth one sentence, not a silent extra object. When they "
        "agree, the group is a SETUP STEP that comes first: create it, then "
        "build the automation or scene against the group entity_id the tool "
