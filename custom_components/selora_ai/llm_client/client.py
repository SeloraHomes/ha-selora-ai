        ...
        *,
        ...
        if refining_scene_context:
            ...
            refining_scene_section = (
                ...
                "The user is EDITING a scene, not operating the house right now. A request\n"
                "to turn a device on/off or set its state (e.g. 'also turn on the TV and\n"
                "the amp, set the amp input to TV') means ADD or UPDATE that device in the\n"
                "scene with the requested state — fold it into the scene proposal. NEVER\n"
                "emit a command intent or run a live device action while refining a scene;\n"
                "return the updated scene, not executed commands.\n"
                "SCALE RULES (YAML only — never mention raw values or scales to the user):\n"
                "- brightness: 0–255. For a percentage use brightness_pct (0-100) "
                "instead and let it be converted — '26%' becomes brightness_pct: 26. "
                "Say '26%' to the user.\n"
